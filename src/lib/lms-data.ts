...
// Persona: học viên mới được cấp tài khoản, chưa học gì cả.
// A1 = đang chờ bắt đầu (tiến độ 0%), các cấp còn lại = khoá.
export const newcomerLevels: Level[] = levels.map((lv) => {
  if (lv.code === "A1") {
    return {
      ...
      status: "in-progress",
      progress: 0,
      courses: lv.courses.map((c) => ({
        ...c,
        progress: 0,
        units: c.units.map((u) => ({
          ...u,
          score: undefined,
          activities: u.activities.map((a) => ({ ...a, done: false })),
        })),
      })),
    };
  }
