    ...
    <div className="rounded-2xl border border-border bg-background p-4">
      <div className="grid gap-3 sm:grid-cols-[260px_1fr]">
        <label className="block">
          <div className="mb-1.5 text-xs font-semibold text-foreground">Đơn vị</div>
          <select
            value={orgId}
          ...
          <p className="mt-1.5 text-[11px] text-muted-foreground">
            Bài thi chỉ thuộc 1 đơn vị. Có thể sao chép sang đơn vị khác sau khi lưu.
          </p>
        </label>
        <div>
          <div className="mb-1.5 flex items-center justify-between">
            <div className="text-xs font-semibold text-foreground">
              ...
              <span className="text-muted-foreground">
                ({classIds.length}/{classesInOrg.length})
              </span>
            </div>
            {classesInOrg.length > 0 && (
              <button
                type="button"
                onClick={() =>
                  onChange(orgId, allSelected ? [] : classesInOrg.map((c) => c.id))
                }
