    ...
    return (
      <>
        <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col border-r border-border/60 bg-background md:flex">
          <Link to={homeFor(role)} className="flex items-center gap-2 border-b border-border/60 px-5 py-4">
            <div
            ...
            {tabs.map((t) => {
              ...
              return (
                <Link
                  ...
                  activeProps={{
                    className:
                      "flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm font-medium text-primary-foreground shadow-soft",
                    style: { background: "var(--gradient-brand)" },
                  }}
                >
          ...
          <div className="relative border-t border-border/60 p-3" ref={ref}>
            <button
              onClick={() => setOpen((v) => !v)}
              className="flex w-full items-center gap-2 rounded-xl bg-surface p-2 ring-1 ring-border hover:bg-muted"
            >
              <div
                className={cn(
              ...
              <div className="min-w-0 flex-1 text-left">
                <div className="truncate text-xs font-semibold text-foreground">{meta.name}</div>
                <div className="truncate text-[10px] text-muted-foreground">{meta.email}</div>
              </div>
              <ChevronDown className="h-3.5 w-3.5 text-muted-foreground" />
            </button>
            {open && (
              <div className="absolute bottom-16 left-3 right-3 rounded-2xl border border-border bg-background p-2 shadow-elevated">
                <button
                ...
                <button
                  onClick={() => { setRole("student"); setOpen(false); }}
                  className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-sm text-foreground hover:bg-muted"
                >
                  <UserRound className="h-4 w-4" /> Học viên {role === "student" && <Check className="ml-auto h-4 w-4 text-primary" />}
                </button>
                <Link
                  to="/teacher"
                  onClick={() => { setRole("teacher"); setOpen(false); }}
                  className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-sm text-foreground hover:bg-muted"
                >
